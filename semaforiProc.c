...
int main (int argc, char** argv){
    ...
    leggi = (sem_t *) mmap(NULL, sizeof(sem_t), PROT_EXEC | PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, 0,0);
    scrivi = (sem_t *) mmap(NULL, sizeof(sem_t), PROT_EXEC | PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, 0,0);

    sem_init(leggi, 1, 0);
    sem_init(scrivi, 1 ,1);

    int test;
    sem_getvalue(leggi, &test);
    printf("valore di leggi è %i\n", test);
    sem_getvalue(scrivi, &test);
    printf("valore di scrivi è %i\n", test);
